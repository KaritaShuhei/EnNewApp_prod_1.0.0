...
class SelectedPostListEditViewController: UIViewController,UITextFieldDelegate, UIScrollViewDelegate, UITextViewDelegate ,DidSelectRowDelegate,ImagePickerMethodDelegate {
    ...
    weak var movieInfomationDelegate : ImagePickerMethodDelegate!
    ...
    func imagePickerDidFinishPickingMediaWithInfo() {
        ...
        //        選択されたらフラグ１を立てる。再生する対象動画をfunc playMovie()で識別。
        pickerFlag = 1
...
//        movieInfomation.movieImageView = movieImageView
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [self] in
            ...
            targetLevelTextField.text = movieInfomation.targetLevel
            titleTextView.text = movieInfomation.title
            introdutionTextView.text = movieInfomation.introduction
    ...
    func sendData(){
        ...
        //ここから動画DB格納定義
        let ref0 = self.firebaseMethod.Ref.child("post").child("\(self.selectedPostID!)")
        let data0 = ["youtubeID":"\(self.youtubeIDTextField.text ?? "")","title":"\(self.titleTextView.text ?? "")","introduction":"\(self.introdutionTextView.text ?? "")","summury":"\(self.summuryTextView.text ?? "")","comment":"\(self.commentTextView.text ?? "")","targetLevel":"\(self.targetLevelTextField.text ?? "")","update_at":"\(date_yyyyMMddHHmmSS)" as Any] as [String : Any]
...
//            let storageReference = Storage.storage().reference().child("post").child("\(self.selectedPostID!)").child("\(self.selectedPostID!).mp4")
...
//
//            }
//            let storageReferenceImage = Storage.storage().reference().child("post").child("\(self.selectedPostID!)").child("\(self.selectedPostID!).png")
//
...
//                //ここから動画DB格納定義
//                let ref0 = self.firebaseMethod.Ref.child("post").child("\(self.selectedPostID!)")
//                let data0 = ["title":"\(self.titleTextView.text ?? "")","introduction":"\(self.introdutionTextView.text ?? "")","summury":"\(self.summuryTextView.text ?? "")","comment":"\(self.commentTextView.text ?? "")","targetLevel":"\(self.targetLevelTextField.text ?? "")","update_at":"\(date_yyyyMMddHHmmSS)" as Any] as [String : Any]
//                ref0.updateChildValues(data0) {
//                    (error:Error?, ref:DatabaseReference) in
//                    if let error = error {
//                        print("Data could not be saved: \(error).")
